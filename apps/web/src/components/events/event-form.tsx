...
export default function EventForm({ accountId, eventId }: EventFormProps) {
  ...
  async function save(nextTab?: Tab) {
    ...
    setError(null)
    try {
      if (!draft.name.trim()) throw new Error(t('イベント名は必須です'))
      if (draft.name.length > 255) throw new Error(t('イベント名は255字以内で入力してください'))
      if (draft.description && draft.description.length > 20000) {
        throw new Error(t('詳細は20000字以内で入力してください'))
      }
      ...
      if (targetType === 'multi-account-dedup' && accountIdsArr.length === 0) {
        throw new Error(t('複数アカウント横断の場合は対象アカを 1 件以上選択してください'))
      }
      ...
      if (eventId) {
        ...
        flashToast(t('イベントを作成しました。続けて予約枠を追加してください。'))
  ...
  async function copyLiffUrl() {
    ...
    try {
      ...
      window.prompt(t('コピーしてください:'), liffUrl)
    }
  }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-4 flex items-center gap-2 text-sm">
        <a href="/events" className="text-blue-600 hover:underline">{t('イベント一覧')}</a>
        <span className="text-gray-400">/</span>
        <span className="text-gray-700">{eventId ? draft.name || t('イベント編集') : t('新規イベント')}</span>
      </div>
      ...
      <div className="mb-4 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {eventId ? draft.name || t('イベント編集') : t('新規イベント作成')}
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {eventId ? t('タブで各項目を編集できます') : t('まず「概要」を保存するとイベントが作成されます')}
          </p>
        </div>
        {eventId && (
          <a
      ...
      {eventId && draft.is_published === 1 && (() => {
        ...
        if (targetType === 'multi-account-dedup') {
          ...
          return (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4 space-y-4">
              <div>
                <div className="text-sm font-medium text-blue-900 mb-2">{t('broadcast 用テンプレ URL')}</div>
                <div className="flex gap-2 items-center">
                  ...
                  >
                    {copiedValue === templateUrl ? t('コピー済') : t('コピー')}
                  </button>
                </div>
                <p className="text-xs text-blue-700 mt-2">
                  {t('broadcast 編集で「リンクするイベント」から選ぶと自動挿入。')}
                  {'{{liff_id}}'} {t('は配信時に各友だちのアカに対応した値に置換されます。')}
                </p>
              </div>
              <div>
                <div className="text-sm font-medium text-blue-900 mb-2">{t('各アカ固定 URL (QR・LP 直貼り用)')}</div>
                <div className="space-y-1.5">
                  {targetAccounts.length === 0 && (
                    <div className="text-xs text-amber-700">{t('対象アカが選択されていません')}</div>
                  )}
                    ...
                    if (!acct.liffId) {
                      return (
                        <div key={a.id} className="text-xs text-amber-700">
                          {acct.country ? acct.country + ' ' : ''}{acct.name}: {t('LIFF ID 未設定')}
                        </div>
                      )
                    }
        ...
        if (liffUrl) {
          return (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              <div className="text-sm font-medium text-blue-900 mb-2">{t('予約 URL（友だちに案内する）')}</div>
              <div className="flex gap-2 items-center">
                <input
                  readOnly
                  value={liffUrl}
                ...
                <button
                  ...
                  className="px-4 py-2 text-sm font-medium bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                >
                  {copiedValue === liffUrl ? t('コピー済') : t('コピー')}
                </button>
              </div>
              <p className="text-xs text-blue-700 mt-2">
        ...
        return (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4 text-xs text-amber-900">
            {t('LIFF ID が未設定のため予約 URL を生成できません。LINE アカウント設定で LIFF ID を登録してください。')}
          </div>
        )
      })()}
      {eventId && draft.is_published === 0 && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4 text-xs text-amber-900">
          {t('現在「下書き」状態です。公開設定タブで「公開する」を ON にすると友だち向けの予約 URL が表示されます。')}
        </div>
      )}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="flex border-b border-gray-200">
          {TABS.map((tabItem) => {
            ...
            const disabled = tabItem.key !== 'overview' && !eventId
            return (
              <button
                key={tabItem.key}
                disabled={disabled}
                onClick={() => !disabled && setTab(tabItem.key)}
                title={disabled ? t('まず「概要」を保存してください') : undefined}
              ...
              >
                <div>{t(tabItem.label)}</div>
                <div className="text-xs font-normal mt-0.5 opacity-80">{t(tabItem.sub)}</div>
              </button>
            )
          })}
        </div>
        ...
        {tab !== 'slots' && (
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
            <div className="text-xs text-gray-500">
              {tab === 'overview' && !eventId && t('保存するとイベントが作成され、予約枠タブに進みます')}
              {tab === 'overview' && eventId && t('変更を「概要を保存」で確定します')}
              {tab === 'publish' && t('「公開する」ON で友だちに予約 URL を案内できます')}
            </div>
            <div className="flex gap-2">
              ...
              >
                {saving ? t('保存中...') : tab === 'overview' && !eventId ? t('イベントを作成') : t(TABS.find((x) => x.key === tab)?.saveLabel ?? '保存')}
              </button>
            </div>
          </div>
        )}
    ...
    <div className="space-y-5">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1.5">
          {t('イベント名')} <span className="text-red-500">*</span>
        </label>
        <input
          type="text"
          value={draft.name}
          ...
          maxLength={255}
          placeholder={t('例: 第1回 AAA 説明会')}
          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('開催場所')}</label>
          <input
            type="text"
            value={draft.venue_name ?? ''}
            onChange={(e) => update('venue_name', e.target.value || null)}
            placeholder={t('例: 渋谷ベース 3F')}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('会場 URL')}</label>
          <input
            type="url"
      ...
      <div>
        ...
        <textarea
          ...
          rows={8}
          placeholder={t('開催趣旨、注意事項、持ち物などを記載...')}
          className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      ...
      <div>
        ...
        >
          <option value="unlimited">{t('制限なし')}</option>
          <option value="1">{t('1 回まで')}</option>
          <option value="2">{t('2 回まで')}</option>
          <option value="3">{t('3 回まで')}</option>
          <option value="5">{t('5 回まで')}</option>
        </select>
      </div>

      {/* 公開対象 */}
      <div className="border-t border-gray-200 pt-5">
        <div className="text-sm font-medium text-gray-700 mb-2">{t('公開対象')}</div>
        <div className="grid grid-cols-2 gap-2 mb-3">
          ...
          >
            <div className="text-sm font-bold">{t('単一アカウント')}</div>
            <div className="text-xs text-gray-600">{t('1 つの LINE アカで運用')}</div>
          </button>
          <button
            type="button"
            onClick={() => {
          ...
          >
            <div className="text-sm font-bold">{t('複数アカウント横断')}</div>
            <div className="text-xs text-gray-600">{t('重複なし配信に対応')}</div>
          </button>
        </div>

        {targetType === 'multi-account-dedup' && (
          <div className="space-y-1.5">
            <div className="text-xs text-gray-600">{t('対象アカ（重複なし配信）')}</div>
            {activeAccounts.length === 0 && (
              <div className="text-sm text-gray-500 italic p-2">{t('アクティブなアカウントがありません')}</div>
            )}
            {activeAccounts.map((a) => {
              ...
              return (
                <label
                  ...
                  title={isCurrent ? t('現在ログイン中のアカウントは必須です') : undefined}
                >
                  <input
                    type="checkbox"
                    checked={checked}
                  ...
                  <span className="text-sm">
                    ...
                    {isCurrent && <span className="ml-1 text-[10px] text-gray-500">{t('(現アカ・必須)')}</span>}
                  </span>
                </label>
              )
            })}
            <div className="text-xs text-gray-500 mt-1">{accountIds.length} {t('件選択中')}</div>
          </div>
        )}
      </div>
    </div>
...
function SlotsTab({
  ...
  async function deleteSlot(slotId: string) {
    if (!eventId) return
    if (!confirm(t('この枠を削除しますか？（既存予約があると削除できません）'))) return
    setBusy(true)
    setErr(null)
    try {
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2 mb-4 items-center justify-between">
        <div className="text-sm text-gray-600">{slots.length} {t('件の予約枠')}</div>
        <div className="flex gap-2">
          <button
      ...
      {slots.length === 0 ? (
        <div className="text-center py-12 text-gray-500 text-sm border border-dashed border-gray-300 rounded-lg">
          {t('予約枠がありません。「＋ 枠を追加」または「📅 一括追加」から作成してください。')}
        </div>
      ) : (
        <div className="overflow-x-auto border border-gray-200 rounded-lg">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-600">
              <tr>
                <th className="text-left px-3 py-2 font-medium">{t('日時')}</th>
                <th className="text-left px-3 py-2 font-medium">{t('定員')}</th>
                <th className="text-left px-3 py-2 font-medium">{t('予約数')}</th>
                <th className="text-left px-3 py-2 font-medium">{t('状態')}</th>
                <th className="text-right px-3 py-2 font-medium">{t('操作')}</th>
              </tr>
            </thead>
            <tbody>
              {slots.map((s) => (
                <tr key={s.id} className="border-t border-gray-200">
                  ...
                  <td className="px-3 py-2 text-right">
                    <button
                      ...
                      title={(s.active_count ?? 0) > 0 ? t('既存予約があるため削除できません') : t('削除')}
        ...
        <BulkSlotDialog
          ...
          onSubmit={async (input) => {
            ...
            if (generated.length === 0) {
              alert(t('生成される枠が0件でした。条件を確認してください。'))
              return
            }
            if (!confirm(`${generated.length}${t('件の枠を生成します。よろしいですか？')}`)) return
...
function AddSlotDialog({
  ...
  async function submit() {
    ...
    try {
      ...
      if (s >= e) throw new Error(t('開始時刻 < 終了時刻'))
      const cap = capacity === '' ? null : Number(capacity)
      if (cap != null && (!Number.isInteger(cap) || cap < 1)) throw new Error(t('定員は1以上の整数'))
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md mx-4">
        <h3 className="text-lg font-bold mb-4 text-gray-900">{t('予約枠を追加')}</h3>
        {err && <div className="bg-red-50 border border-red-200 text-red-700 p-2 rounded-lg mb-3 text-sm">{err}</div>}
        <div className="space-y-3">
          <label className="block">
            <span className="text-sm font-medium text-gray-700">{t('日付（JST）')}</span>
            <input
              type="date"
              value={date}
          ...
          <div className="grid grid-cols-2 gap-3">
            <label>
              <span className="text-sm font-medium text-gray-700">{t('開始')}</span>
              <input
                type="time"
                value={startTime}
            ...
            <label>
              <span className="text-sm font-medium text-gray-700">{t('終了')}</span>
              <input
                type="time"
                value={endTime}
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">{t('定員（空欄=無制限）')}</span>
            <input
              type="number"
              min={1}
              value={capacity}
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4">
        <h3 className="text-lg font-bold mb-4 text-gray-900">{t('予約枠の一括追加')}</h3>
        {err && <div className="bg-red-50 border border-red-200 text-red-700 p-2 rounded-lg mb-3 text-sm">{err}</div>}
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <label>
              <span className="text-sm font-medium text-gray-700">{t('開始日')}</span>
            ...
            <label>
              <span className="text-sm font-medium text-gray-700">{t('終了日')}</span>
          ...
          <div>
            <span className="text-sm font-medium text-gray-700 block mb-1.5">{t('時刻パターン')}</span>
            {patterns.map((p, i) => (
          ...
          <label className="block">
            <span className="text-sm font-medium text-gray-700">{t('定員（各枠共通・空欄=無制限）')}</span>
            <input
              type="number"
              min={1}
              value={capacity}
  ...
  return (
    <div className="space-y-5">
      <label className="flex items-start gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
        ...
        />
        <div>
          <div className="text-sm font-medium text-gray-900">{t('承認制')}</div>
          <div className="text-xs text-gray-500 mt-0.5">
            {t('ON: 友だちが予約しても運営が「承認」するまで未確定')}<br />
            {t('OFF: 定員空きがあれば即時確定')}
          </div>
        </div>
      </label>
      ...
      <div>
        ...
        <select
          ...
          className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          <option value="disabled">{t('不可（運営に LINE 連絡）')}</option>
          <option value="0">{t('直前まで可')}</option>
          <option value="6">{t('6 時間前まで')}</option>
          <option value="12">{t('12 時間前まで')}</option>
          <option value="24">{t('24 時間前まで')}</option>
          <option value="48">{t('48 時間前まで')}</option>
        </select>
      </div>
      ...
      <label className="flex items-start gap-3 p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
        ...
        />
        <div>
          <div className="text-sm font-medium text-gray-900">{t('前日リマインダ')}</div>
          <div className="text-xs text-gray-500 mt-0.5">{t('前日 18:00 JST に LINE で通知')}</div>
        </div>
      </label>
      ...
      <div>
        ...
        <select
          ...
          className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
          <option value="off">{t('送信しない')}</option>
          <option value="1">{t('1 時間前')}</option>
          <option value="2">{t('2 時間前')}</option>
          <option value="3">{t('3 時間前')}</option>
          <option value="6">{t('6 時間前')}</option>
          <option value="24">{t('24 時間前')}</option>
        </select>
      </div>
      ...
      <div className="border-t border-gray-200 pt-5 space-y-4">
        <div>
          <div className="text-sm font-medium text-gray-900 mb-1">{t('予約者向けカスタムメッセージ')}</div>
          <p className="text-xs text-gray-500">
            {t('予約者だけに届く LINE 通知の末尾に追加されます（Zoom URL など）。空欄ならデフォルト文言のみ。')}
          </p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            maxLength={2000}
            placeholder={t('例: 当日の Zoom URL: https://us02web.zoom.us/j/...')}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <p className="text-xs text-gray-500 mt-1">{t('確定通知（即時 / 後追い承認）の末尾に追加')}</p>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            maxLength={2000}
            placeholder={t('例: 開始 10 分前に同じ URL からご入室ください')}
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <p className="text-xs text-gray-500 mt-1">{t('前日 / N 時間前のリマインド末尾に追加')}</p>
        </div>
      </div>
      ...
      <div>
        <div className="text-sm font-medium text-gray-700 mb-2">{t('公開状態')}</div>
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            <div className="text-sm font-bold text-gray-900">{t('下書き')}</div>
            <div className="text-xs text-gray-600 mt-0.5">{t('友だちには見えない')}</div>
          </button>
          <button
            type="button"
          ...
          >
            <div className="text-sm font-bold text-gray-900">{t('公開する')}</div>
            <div className="text-xs text-gray-600 mt-0.5">{t('予約 URL が有効になる')}</div>
          </button>
        </div>
        <p className="text-xs text-gray-500 mt-2">
          {draft.is_published === 1
            ? t('✓ 保存後、友だちに「予約 URL」を案内できます。')
            : t('保存しても友だちには表示されません。')}
        </p>
      </div>
