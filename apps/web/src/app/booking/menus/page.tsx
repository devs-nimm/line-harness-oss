...
export default function MenusPage() {
  ...
  async function remove(id: string) {
    if (!selectedAccountId) return
    if (!confirm(t('このメニューを削除しますか？（既存予約は維持されます）'))) return
  ...
  return (
    <div>
      ...
      {!selectedAccountId ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center text-sm text-gray-500">
          {t('まだメニューがありません。右上の「+ 新規メニュー」から追加してください。')}
        </div>
      ) : (
            ...
            <table className="w-full min-w-[640px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">{t('所要')}</th>
                  ...
                  <tr key={m.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="inline-flex gap-2 text-xs">
                        <button onClick={() => setEditing(m)} className="text-blue-600 hover:underline">{t('編集')}</button>
                        <Link href={`/booking/menus/staff?menu_id=${m.id}`} className="text-blue-600 hover:underline">
                          {t('スタッフ割当')}
                        </Link>
                        {!liffId ? (
                          <span className="text-gray-300" title={t('LIFF ID 未設定')}>{t('専用URL')}</span>
                        ) : !m.is_active ? (
                          ...
                          // 有効化されるまでコピー不可にする。
                          <span className="text-gray-300" title={t('メニューを有効化するとコピーできます')}>{t('専用URL')}</span>
                        ) : (
                          <button
                            type="button"
                          ...
                          >
                            {copiedMenuId === m.id ? `✓ ${t('コピー済')}` : t('専用URL')}
                          </button>
                        )}
                        <button onClick={() => remove(m.id)} className="text-red-600 hover:underline">{t('削除')}</button>
                      </div>
                    </td>
                  </tr>
                ))}
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-base font-semibold">{form.id ? t('メニュー編集') : t('新規メニュー')}</h2>
        </div>
        <div className="px-6 py-4 space-y-4">
          <Field label={t('名前')} required>
            <input
              type="text"
              value={form.name ?? ''}
          ...
          <Field label={t('予約申込時に自動付与するタグ')}>
            ...
            <p className="mt-1 text-xs text-gray-500">
              {t('このメニューが予約されると、申込者の友だちに自動でこのタグが付きます。タグは既存のものから選択してください (友だち画面 / シナリオ等で使われているタグ)。')}
            </p>
          </Field>
